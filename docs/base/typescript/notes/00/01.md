...
TypeScript 不需要显示的实现接口，而是编译器会对比接口的类的字段，只有具有接口的属性就默认实现了这个接口，这是隐式实现，或者加上 implements 显示实现
```typescript
interface Employee {
    name: string
    salary: number
...
const emp1: Employee = empImpl // 属性包含 Employee 接口属性，自动实现 Employee 接口
```
...
// service.ts文件
// 实现接口
class RPCService implements Service { // 接口定义和实现是同一个人，使用显示实现，但是这是不推荐的！
    login(): void {
...
```
显示实现例子中在登录页面还能使用接口中非 login() 方法，登陆页面只要 login() 方法，所以更好的方法是使用者来定义方法。
